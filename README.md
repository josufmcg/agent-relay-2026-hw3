...
attempts in the containerized deployment, while workers execute tasks on their
...
Open <http://127.0.0.1:8000/> for the token-based local dashboard. Use
...
`database.py` contains SQLAlchemy models and database setup. `storage.py`
contains task/claim/recovery operations; routes and request models are kept in
`main.py` and `schemas.py`. PostgreSQL uses `FOR UPDATE SKIP LOCKED` for
concurrent claims. SQLite remains available as the default for local tests by
setting `RELAY_DATABASE_URL` explicitly.
...
The Compose deployment includes PostgreSQL, but does not include Kubernetes,
CI, external brokers, or an LLM.
